...
if %errorlevel% neq 0 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference='SilentlyContinue'; $ErrorActionPreference='Stop'; try { $zip = Join-Path $env:PS_TEMP $env:PS_ZIP; $extract = Join-Path $env:PS_TEMP 'node-extract'; if (Test-Path $extract) { Remove-Item $extract -Recurse -Force }; Invoke-WebRequest -Uri $env:PS_URL -OutFile $zip -UseBasicParsing; $actual = (Get-FileHash -Path $zip -Algorithm SHA256).Hash.ToLower(); $expected = $env:PS_SHA256.ToLower(); if ($actual -ne $expected) { Remove-Item $zip -Force -ErrorAction SilentlyContinue; throw \"SHA256 mismatch for $env:PS_ZIP. Expected $expected, got $actual.\" }; Expand-Archive -Path $zip -DestinationPath $extract -Force; if (Test-Path $env:PS_DEST) { Remove-Item $env:PS_DEST -Recurse -Force }; Move-Item -Path (Join-Path $extract $env:PS_DIR) -Destination $env:PS_DEST -Force; Remove-Item $zip -Force; Remove-Item $extract -Recurse -Force; exit 0 } catch { Write-Host $_.Exception.Message; exit 1 }"
